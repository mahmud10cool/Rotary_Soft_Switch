...
for tfor = 1:mm
    ...
    % Current revolutions
    theta_1 = ((p1-P_M)*V1_0*2*pi)/(beta*hyd_D);
    
    % Flow to the chamber via the pump/motor
    Q_hyd_1 = (hyd_D/2*pi)*thetadot;
    ...
    % Current revolutions
    theta_2 = (((p1+0.5*ddt*l1)-P_M)*V1_0*2*pi)/(beta*hyd_D);
    
    % Flow to the chamber via the pump/motor
    Q_hyd_2 = (hyd_D/2*pi)*(thetadot+0.5*ddt*m1);
    
    % Hydraulic Torque   
    hyd_torque_2 = (hyd_D/(2*pi))*((p2+0.5*ddt*k1) - (p1+0.5*ddt*l1));
    ...
    % Current revolutions
    theta_3 = (((p1+0.5*ddt*l2)-P_M)*V1_0*2*pi)/(beta*hyd_D);
    
    % Flow to the chamber via the pump/motor
    Q_hyd_3 = (hyd_D/2*pi)*(thetadot+0.5*ddt*m2);
    
    % Hydraulic Torque
    hyd_torque_3 = (hyd_D/(2*pi))*((p2+0.5*ddt*k2) - (p1+0.5*ddt*l2));
    ...
    %% Fourth term
    Q4 = Cd*A_vt_options.*sqrt((2/rho)*abs(P_H-(p2+ddt*k3))).*sign(P_H-(p2+ddt*k3));
    
    % Current revolutions
    theta_4 = (((p1+ddt*l3)-P_M)*V1_0*2*pi)/(beta*hyd_D);
    
    % Flow to the chamber via the pump/motor
    Q_hyd_4 = (hyd_D/2*pi)*(thetadot+ddt*m3);
    
    % Hydraulic Torque
    hyd_torque_4 = (hyd_D/(2*pi))*((p2+ddt*k3) - (p1+ddt*l3));
    ...
    throttling_1 = Q1 .* (P_H-p2);
    
    throttling_2 = Q2 .* (P_H-(p2+0.5*ddt*k1));
    
    throttling_3 = Q3 .* (P_H-(p2+0.5*ddt*k2));
    
    throttling_4 = Q4 .* (P_H-(p2+ddt*k3));
    
    energy_throttle_loss = energy_throttle_loss + (ddt/6)*(throttling_1+2*throttling_2+2*throttling_3+throttling_4);
    
    %% Integration
    p2 = p2 + (ddt/6)*(k1 + 2*k2 + 2*k3 + k4);
    p1 = p1 + (ddt/6)*(l1 + 2*l2 + 2*l3 + l4);
    thetadot = thetadot + (ddt/6)*(m1 + 2*m2 + 2*m3 + m4);
...
for t_f = 1:1:n_t-1
    ...
    for tfor = 1:mm
        ...
        Q_hyd_1_f = (hyd_D/(2*pi))*thetadotf;
    
        hyd_torque_1_f = (hyd_D/(2*pi))*(p2f - p1f);
        
        P2dot_f = (beta/(V2_0))*(Q1_f - Q_hyd_1_f);
        P1dot_f = (beta/(V1_0)) * (Q_hyd_1_f);
        ...
        Q_hyd_2_f = (hyd_D/(2*pi))*(thetadotf+0.5*ddt*m1_f);
    
        hyd_torque_2_f = (hyd_D/(2*pi))*((p2f+0.5*ddt*k1_f) - (p1f+0.5*ddt*l1_f));
        ...
        Q_hyd_3_f = (hyd_D/(2*pi))*(thetadotf+0.5*ddt*m2_f);
    
        hyd_torque_3_f = (hyd_D/(2*pi))*((p2f+0.5*ddt*k2_f) - (p1f+0.5*ddt*l2_f));
        ...
        Q_hyd_4_f = (hyd_D/(2*pi))*(thetadotf+ddt*m3_f);
    
        hyd_torque_4_f = (hyd_D/(2*pi))*((p2f+ddt*k3_f) - (p1f+ddt*l3_f));
        ...
        %% Throttling losses
        throttling_1_f = Q1_f .* (P_H-p2f);

        throttling_2_f = Q2_f .* (P_H-(p2f+0.5*ddt*k1_f));

        throttling_3_f = Q3_f .* (P_H-(p2f+0.5*ddt*k2_f));

        throttling_4_f = Q4_f .* (P_H-(p2f+ddt*k3_f));

        energy_throttle_loss_f = energy_throttle_loss_f + (ddt/6)*(throttling_1_f+2*throttling_2_f+2*throttling_3_f+throttling_4_f);

        %% Integration
        p2f = p2f + (ddt/6)*(k1_f + 2*k2_f + 2*k3_f + k4_f);
        p1f = p1f + (ddt/6)*(l1_f + 2*l2_f + 2*l3_f + l4_f);
        thetadotf = thetadotf + (ddt/6)*(m1_f + 2*m2_f + 2*m3_f + m4_f);
    end

    %% Theta
    P2next_f = p2f;
    P1next_f = p1f;
    theta_dot_next_f = thetadotf;
...
sgtitle('Final Modified RK Method')
...
savefig(plot_fig,'Rotary_dp_rk_modified')
